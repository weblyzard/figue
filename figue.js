...
// Source: gitlab@gitlab.semanticlab.net/infovis/figue.git#master

const compareArray = function(sourceArr, testArr) {
	if (sourceArr.length !== testArr.length) return false;
	for (var i = 0; i < testArr.length; i++) {
		if (sourceArr[i].compare) {
			if (!sourceArr[i].compare(testArr[i])) return false;
		}
		if (sourceArr[i] !== testArr[i]) return false;
	}
	return true;
};

const figue = (function() {
	...
	function manhattanDistance(vec1, vec2) {
		var N = vec1.length;
		var d = 0;
		for (var i = 0; i < N; i++) d += Math.abs(vec1[i] - vec2[i]);
		return d;
	}
	...
	function maxDistance(vec1, vec2) {
		var N = vec1.length;
		var d = 0;
		for (var i = 0; i < N; i++) d = Math.max(d, Math.abs(vec1[i] - vec2[i]));
		return d;
	}
	...
	function addVectors(vec1, vec2) {
		var N = vec1.length;
		var vec = new Array(N);
		for (var i = 0; i < N; i++) vec[i] = vec1[i] + vec2[i];
		return vec;
	}
	...
	function multiplyVectorByValue(value, vec) {
		var N = vec.length;
		var v = new Array(N);
		for (var i = 0; i < N; i++) v[i] = value * vec[i];
		return v;
	}
	...
	function putString(str, width, index) {
		...
		return (
			repeatChar(' ', index) +
			str +
			repeatChar(' ', width - (str.length + index))
		);
	}
	...
	function generateDendogram(
		...
		var lines = new Array();
		var centroidstr = prettyValue(tree.id); //prettyVector(tree.centroid)
		if (tree.isLeaf()) {
			var labelstr = String(tree.label);
			var len = 1;
			if (withCentroid) len = Math.max(centroidstr.length, len);
			if (withLabel) len = Math.max(labelstr.length, len);

			lines.push(centerString('|', len));
			if (withCentroid) lines.push(centerString(centroidstr, len));
			if (withLabel) lines.push(centerString(labelstr, len));
		} else {
			var distancestr = prettyValue(tree.dist); //
			var left_dendo = generateDendogram(
				tree.left,
				sep,
				balanced,
				withLabel,
			...
			var right_dendo = generateDendogram(
				tree.right,
				sep,
				balanced,
				withLabel,
			...
			if (withCentroid) len = Math.max(centroidstr.length, len);
			if (withDistance) len = Math.max(distancestr.length, len);

			// calculate position of new vertical bar
			var bar_ix =
				left_bar_ix +
				Math.floor(
					(left_dendo[0].length - left_bar_ix + sep + (1 + right_bar_ix)) / 2
				);
			...
			if (withCentroid) {
				lines.push(
					putString(
						centroidstr,
						len,
						bar_ix - Math.floor(centroidstr.length / 2)
					)
				); //centerString (centroidstr , len)) ;
			}
			if (withDistance) {
				lines.push(
					putString(
						distancestr,
						len,
						bar_ix - Math.floor(distancestr.length / 2)
					)
				); //centerString (centroidstr , len)) ;
			}
			...
			var hlineLen =
				sep + (left_dendo[0].length - left_bar_ix) + right_bar_ix + 1;
			...
			for (i = 0; i < Math.max(left_dendo.length, right_dendo.length); i++) {
				var left = '';
				if (i < left_dendo.length) left = left_dendo[i];
				else left = repeatChar(' ', left_dendo[0].length);

				var right = '';
				if (i < right_dendo.length) right = right_dendo[i];
				else right = repeatChar(' ', right_dendo[0].length);
	...
	function agglomerate(labels, vectors, distance, linkage) {
		...
		var c1, c2, c1Cluster, c2Cluster, i, j, p, root, newCentroid, newCluster;
	...
	function kmeans(k, vectors) {
		...
		while (repeat) {
			// assignment step
			for (var j = 0; j < k; j++) clusterSizes[j] = 0;
			...
			for (let j = 0; j < k; j++) newCentroids[j] = null;
			...
			for (let j = 0; j < k; j++) {
				newCentroids[j] = multiplyVectorByValue(
					1 / clusterSizes[j],
					newCentroids[j]
				);
			}
			...
			// check nb of iters
			if (nb_iters > figue.KMEANS_MAX_ITERATIONS) repeat = false;
		}
	...
	function fcmeans(k, vectors, epsilon, fuzziness) {
		...
		while (repeat) {
			...
			// check nb of iters
			if (nb_iters > figue.FCMEANS_MAX_ITERATIONS) repeat = false;
		}
...
figue.Node.prototype.buildDendogram = function(
	sep,
	balanced,
	withLabel,
	withCentroid,
	...
	var lines = figue.generateDendogram(
		this,
		sep,
		balanced,
		withLabel,
