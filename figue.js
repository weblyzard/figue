...
var figue = (function() {
  ...
  function prettyVector(vector) {
    ...
    for (var i = 0; i < vector.length; i++) vals[i] = Math.round(vector[i] * precision) / precision;
    return vals.join(',');
  }
  ...
  function kmeans(k, vectors) {
    ...
    while (repeat) {
      ...
      for (j = 0; j < k; j++) newCentroids[j] = null;
    ...
    while (repeat) {
      ...
      for (i = 0; i < membershipMatrix.rows; i++) {
        for (j = 0; j < k; j++) {
          ...
          if (membershipMatrix.mtx[i][j] > 0) membershipMatrix.mtx[i][j] = 1 / membershipMatrix.mtx[i][j];
        }
      }
